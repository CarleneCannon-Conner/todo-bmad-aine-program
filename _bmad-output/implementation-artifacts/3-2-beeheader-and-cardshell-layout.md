...
- [ ] Task 1: Copy bee SVG to frontend assets (AC: #1)
  - [ ] Copy `assets/bumble-bee.svg` to `frontend/public/bumble-bee.svg` (served statically)
  - [ ] OR inline the SVG in the BeeHeader component (smaller, no extra request)
  - [ ] Clean up SVG: remove Inkscape/Sodipodi metadata, keep essential paths only
  - [ ] Set reasonable display size (e.g., width 80-120px, centered)
- [ ] Task 2: Create `BeeHeader` component (AC: #1, #5)
  - [ ] Create `frontend/src/components/BeeHeader/` with `index.ts`, `BeeHeader.tsx`, `BeeHeader.css`, `BeeHeader.test.tsx`
  - [ ] Render bee SVG prominently (as `<img>` or inline SVG)
  - [ ] Render `<h1>my todos</h1>` below the bee
  - [ ] Style: centered, `color: var(--color-text)`, Patrick Hand font (inherited from body)
  - [ ] No interactive behaviour — purely decorative
  - [ ] Bee SVG: `aria-hidden="true"` (decorative image)
- [ ] Task 3: Create `CardShell` component (AC: #2, #3, #4, #5)
  - [ ] Create `frontend/src/components/CardShell/` with `index.ts`, `CardShell.tsx`, `CardShell.css`, `CardShell.test.tsx`
  - [ ] Props: `children: React.ReactNode`
  - [ ] Render a wrapper `<div>` (or `<main>`) that contains children
  - [ ] Mobile-first base styles: full-bleed, `background: var(--color-background)`, no border-radius, no shadow
  - [ ] Desktop media query: `@media (min-width: 768px)` adds card treatment
- [ ] Task 4: Style CardShell responsive layout (AC: #2, #3, #4)
  - [ ] Mobile base: `min-height: 100vh`, `padding: 1rem`, `background: var(--color-background)`
  - [ ] Desktop `@media (min-width: 768px)`:
    - [ ] `max-width: 560px`
    - [ ] `margin: 2rem auto`
    - [ ] `border-radius: 20px`
    - [ ] `box-shadow: 0 4px 24px rgba(61, 46, 31, 0.18), 0 1px 4px rgba(61, 46, 31, 0.10)`
    - [ ] `min-height: auto` (card doesn't fill viewport)
  - [ ] Body/html background: `var(--color-desktop-bg)` on desktop (Story 3.1 set body bg to `--color-background`, this story needs to handle the desktop outer bg)
- [ ] Task 5: Update `App.tsx` to use BeeHeader and CardShell (AC: #1, #2, #3)
  - [ ] Import and render `BeeHeader` at top
  - [ ] Wrap all content inside `CardShell`
  - [ ] Remove old `.app` max-width/margin styles from `App.css` (CardShell handles layout now)
  - [ ] Structure: `<CardShell><BeeHeader /><TaskInput .../><TaskList .../></CardShell>`
- [ ] Task 6: Handle desktop background colour (AC: #3)
  - [ ] On desktop (>= 768px), the outer page background should be `var(--color-desktop-bg)` (Honey Oak)
  - [ ] On mobile (< 768px), page background is `var(--color-background)` (cream)
  - [ ] Approach: use media query on `body` in `index.css` to switch background
  - [ ] OR: CardShell handles its own background while body stays cream
- [ ] Task 7: Create `BeeHeader.test.tsx` (AC: #5)
  - [ ] Test: renders bee image/SVG element
  - [ ] Test: renders "my todos" heading text
- [ ] Task 8: Create `CardShell.test.tsx` (AC: #5)
  - [ ] Test: renders children content passed to it
- [ ] Task 9: Clean up old App.css layout styles
  - [ ] Remove `.app` max-width, margin, padding (replaced by CardShell)
  - [ ] Remove `.app h1` styles (replaced by BeeHeader)
